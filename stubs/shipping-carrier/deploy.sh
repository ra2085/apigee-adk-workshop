...
DEFAULT_IMAGE_URL="gcr.io/apigee-hw/shipping-carrier-service:latest"
DEFAULT_SERVICE_NAME="shipping-carrier-service" # Default service name if IMAGE_NAME is not provided
...
SHIPPING_API_CR_URL=$(gcloud run services describe "${TARGET_SERVICE_NAME}" \
    --platform="managed" \
    --region="${REGION}" \
    --format="value(status.url)")

export SHIPPING_API_CR_URL

echo "Service URL: ${SHIPPING_API_CR_URL}"
echo "The service URL has also been exported as SHIPPING_API_CR_URL."