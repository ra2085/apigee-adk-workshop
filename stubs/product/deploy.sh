...
DEFAULT_IMAGE_URL="gcr.io/apigee-hw/product-service:latest"
DEFAULT_SERVICE_NAME="product-service" # Default service name if IMAGE_NAME is not provided
...
PRODUCTS_API_CR_URL=$(gcloud run services describe "${TARGET_SERVICE_NAME}" \
    --platform="managed" \
    --region="${REGION}" \
    --format="value(status.url)")

export PRODUCTS_API_CR_URL

echo "Service URL: ${PRODUCTS_API_CR_URL}"
echo "The service URL has also been exported as PRODUCTS_API_CR_URL."