...
DEFAULT_IMAGE_URL="gcr.io/apigee-hw/orders-service:latest"
DEFAULT_SERVICE_NAME="orders-service" # Default service name if IMAGE_NAME is not provided
...
ORDERS_API_CR_URL=$(gcloud run services describe "${TARGET_SERVICE_NAME}" \
    --platform="managed" \
    --region="${REGION}" \
    --format="value(status.url)")

export ORDERS_API_CR_URL

echo "Service URL: ${ORDERS_API_CR_URL}"
echo "The service URL has also been exported as ORDERS_API_CR_URL."