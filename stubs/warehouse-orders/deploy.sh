...
DEFAULT_IMAGE_URL="gcr.io/apigee-hw/warehouse-orders-service:latest"
DEFAULT_SERVICE_NAME="warehouse-orders-service" # Default service name if IMAGE_NAME is not provided
...
WAREHOUSE_API_CR_URL=$(gcloud run services describe "${TARGET_SERVICE_NAME}" \
    --platform="managed" \
    --region="${REGION}" \
    --format="value(status.url)")

export WAREHOUSE_API_CR_URL

echo "Service URL: ${WAREHOUSE_API_CR_URL}"
echo "The service URL has also been exported as WAREHOUSE_API_CR_URL."