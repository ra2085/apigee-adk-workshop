...
import { McpApi } from "./api.js"; // Assuming the api file is mcp-api.ts
import { SSEServerTransport } from "@modelcontextprotocol/sdk/server/sse.js";
import { StdioServerTransport } from "@modelcontextprotocol/sdk/server/stdio.js";
import { generateOpenApiTools } from "./openapi_tools.js";
import { executeOpenApiTool } from "./mcp_api_executor.js"; // Renamed from mcp_api_executor.ts
/**
 * Main MCP server class for the custom MCP API integration.
 */
class ApiProductMcpServer extends McpServer {
    api;
    openApiTools = []; // Store generated tools
    constructor(options = {}) {
        super({
    ...
    _registerDynamicTools() {
        ...
        this.openApiTools.forEach(tool => {
            this.tool(tool.method, tool.description, tool.parameters.shape, // Zod schema from the tool definition
            async (params, _extra) => {
...
async function main() {
    ...
    if (mcpMode === 'STDIO') {
        ...
        // Initialization requires credentials, which we've already validated.
        // Note: Initialization might fail if credentials are required for spec fetching.
        // Consider if initialization should be skipped or handled differently in STDIO mode
        // if spec fetching *requires* credentials. For now, we attempt initialization.
        try {
        ...
        app.get("/sse", (req, res) => {
            ...
            server.connect(transport); // Connect the server when a client connects via SSE
        });
        ...
        app.listen(port, () => {
            ...
            console.log(`POST messages endpoint available at http://localhost:${port}/messages`);
